...
import { Plus, Edit2, Trash2, CheckCircle, Clock, AlertCircle, X, Check } from 'lucide-react';
...
export default function TaskManager() {
    ...
    const confirmDelete = async () => {
        ...
        try {
            const response = await fetch(`${API_BASE}/tasks/${deletingTask.id}`, {
                method: 'DELETE'
            });
        ...
        <div className="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100">
            ...
            <div className="fixed top-4 right-4 z-50 space-y-2">
                {toasts.map((toast) => (
                    <div
                        key={toast.id}
                        className={`flex items-center gap-2 px-4 py-3 rounded-lg border shadow-lg min-w-80 animate-in slide-in-from-right-2 ${getToastColor(toast.type)}`}
                    >
